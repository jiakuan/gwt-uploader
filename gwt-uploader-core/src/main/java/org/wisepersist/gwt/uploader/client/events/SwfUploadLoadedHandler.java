 ...
 * </pre>
 *
 * <p>Note that the 'swfUploadLoaded' event is fired by the Uploader component's internal
 * 'flashReady' event to let you know that it is safe to call Uploader methods.  Also note that
 * these events are currently only fired when the Uploader component determines that the browser
 * does not support the Ajax upload technique and is therefore utilizing the SWFUpload/Flash
 * technique for handling the upload.  If you need to determine which state the Uploader component
 * is in, you can make use of the {@link Uploader#isAjaxUploadWithProgressEventsSupported()}
 * method.
 *
 * <p>See the documentation on the {@link SwfUploadLoadedEvent} class for more details on the data
...
public interface SwfUploadLoadedHandler {
   ...
   */
  boolean onSwfUploadLoaded(final SwfUploadLoadedEvent swfUploadLoadedEvent);
